...
```{r}
ggmap(philippines_map)+
geom_point(aes(x=longitude,y=latitude, col=event_type),data=ph16_23,alpha = 1) +
scale_size(range = c(0.1, 2)) +
  theme_void() +
labs(title = "Event Types in Between 2016-2023",
       subtitle = "30 June 2016 - 30 June 2023",
       caption = "source: https://acleddata.com/")
```

```{r}
# Create raster for high density regions
rasterphil <- ne_states(country = "Philippines", returnclass = "sf")

selected_regions <- rasterphil %>%
  filter(region %in% c("National Capital Region", "Western Visayas (Region VI)", "Northern Mindanao (Region X)"))
selected_regions <- selected_regions[c(14, 20, 9), ] 
selected_regions_1 <- selected_regions %>% filter(grepl('PH-NEC', iso_3166_2)) %>% mutate(col_regions_raster = '#f8bc94')
selected_regions_2 <- selected_regions %>% filter(grepl('PH-BUK', iso_3166_2)) %>% mutate(col_regions_raster = '#98b4bc')
selected_regions_3 <- selected_regions %>% filter(iso_3166_2 == "PH-MNL") %>% mutate(col_regions_raster = 'green')
...
mapview(mymap, color = "black", col.regions = "#103444",
  ...
  mapview(selected_regions_1, layer.name = "Western Wisayas", color = "black", col.regions = '#f8bc94', lwd = 2) +
  mapview(selected_regions_2, layer.name = "Northern Mindanao", color = "black", col.regions = '#98b4bc', lwd = 2) +
  mapview(selected_regions_3, layer.name = "Manila", color = "white", col.regions = 'green', lwd = 2)
...
unique_months <- events_per_month %>%
  pull(month)
# Plot the histogram
ggplot(events_per_month, aes(x = month, y = num_events)) +
